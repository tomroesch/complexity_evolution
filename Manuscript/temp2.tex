...
\begin{document}
	...
	\begin{figure}
		\includegraphics[width=\linewidth]{figure1_part.pdf}\caption{(A) Two dimensional fitness landscape, which binding site length on the x-axis, and intensive binding energy $\gamma=E/l$ on the y-axis. (B) The fitness landscape for fixed binding site length (blue) and the exponential approximation (orange).}\label{fig1}
	\end{figure}
	...
	In the low mutation rate limit, an exact steady state distribution can be numerically computed from substitution rates (Methods or Supplementary). In the regime of small selection coefficients $Ns\sim1$, an analytic steady state can be approximated (Methods or Supplementary), which is equal to the result in equation \ref{equ:steady_state}.
    	...
    	\begin{figure}[H]
    	\centering
    	\includegraphics[width=.5\linewidth]{genetic_load.pdf}\caption{This could be a figure to show what the genetic load is.}\label{fig:load_theory}
    \end{figure}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{length_scaling.pdf}
...
\paragraph*{Supplementary Methods~2}
\label{supp2}
{\bf Generic Mutation Drift and Driver mutations.}\\
Here we use the population genetic notation of quantitative traits \cite{nourmohammad2013evolution}, and give a general form of the mutation drift. In addition we compute the contribution of driver mutations to the mutation drift.\\
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{supp4_productlog_scaling.pdf}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=\linewidth]{supp4_scalings.pdf}
%	\caption{Note: All these scaling assume that $\Delta_E\sim\left(\frac{n}{n-1}\right)^2$.(A) Mutation selection balance in equilibrium, 
%	computed as maximum from distributions with Binomal (blue) or Gaussian (green) discrete neutral distribution , compared to prediction from Held 2019. (B) Same as in (A), but for non-equilibrium. (C) Scaling of mutation selection balance of a continuous distribution compared to Held 2019. 
%	(D) Scaling of genetic Load from sigmoid fitness function, comparing discrete distributions with Held 2019 prediction. (E) Scaling of genetic load of continuous 
%%	distribution compared to prediction from Held 2019.}
%\end{figure}
